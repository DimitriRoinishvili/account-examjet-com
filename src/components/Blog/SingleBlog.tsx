...
const SingleBlog = ({blog}: { blog: Blog }) => {
    const author = authorData.find((author) => author.id === blog.authorId);
    const {title, image, paragraph, tags, publishDate, path} = blog;
        ...
        <Link href={`/blog${path}`}>
            <div
                ...
                <div className="relative block aspect-[37/22] w-full">
                    ...
                    {/*    {tags[0]}*/}
                    {/*</div>*/}
                    <Image src={`${BASE_PATH}/${image}`} alt={title} fill objectFit="contain"/>
                </div>
                <div
                    className="p-6 sm:p-8 md:px-6 md:py-8 lg:p-8 xl:px-5 xl:py-8 2xl:p-8 flex flex-col justify-between h-[350px]">
                    <div>
                        <h3 className="mb-4 block text-xl font-bold text-black hover:text-primary dark:text-white dark:hover:text-primary sm:text-2xl line-clamp-2">
                            {title}
                        </h3>
                            ...
                            <div
                                ...
                                <div className="mr-4">
                                    <div className="relative h-10 w-10 overflow-hidden rounded-full">
                                        <Image src={`${BASE_PATH}/${author.image}`} alt={author.name} fill/>
                                    </div>
                                </div>
